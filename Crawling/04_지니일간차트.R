...
query <- '?ditc=D&ymd=20210509&genrecode=M0100&pg=' # 0509오늘날짜
url <- paste0(base_url, query, '1') # 그냥 숫자를 1로 주고 끄집어 냄
html <- read_html(url)
...
len <- length(trs)
len
tr <- trs[24] #어떤 것을 하나 잡아서 testing을 함
tr
num <- html_node(tr, '.number') %>% html_text() 
num
nums <- unlist(str_split(num, '\r\n')) # 23과 new space가 리스트형태로 나옴
nums
rank <- as.integer(nums[1]) 
move <- str_trim(nums[2])  #move를 trim함
move
if (move=='new') {
    last <- 999
} else { 
...
last
title <- html_node(tr, '.title.ellipsis') %>% html_text() # title.ellopsis의 타이틀을 추출
title
title <- gsub('\r\n', '', title)
title
