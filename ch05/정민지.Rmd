...
```{r}
mean((mpg_new %>%
    filter(class =='suv'))$cty)
mean((mpg_new %>%
    filter(class =='compact'))$cty)
...
```{r}
mpg %>% select(class,cty) %>% group_by(class) %>% summarise(cty_mean=mean(cty)) %>% head(3)
```
...
mpg %>%
    group_by(class) %>%
    summarize(t_avg=mean(cty)) %>%
    arrange(desc(t_avg)) %>% head(7)    
```
...
```{r}
mpg %>% filter(class =="compact") %>% group_by(manufacturer) %>% summarise(n=n()) %>% arrange(desc(n)) %>% head(mpg)
```
