...
#1. plot
head(cars)
plot(cars,type='p',main='cars') #양의 상관관계에 있다(p-point, main-title)
...
gapminder %>% 
    ...
    geom_histogram() #위로 쌓는 stack이 기본
...
gapminder %>% 
    ...
    geom_bar(stat='identity',aes(fill=continent)) +
    scale_fill_brewer(palette = 'Blues') #오름차순
...
gapminder %>% 
    ...
    ggplot(aes(reorder(continent,-n),n)) +
    geom_bar(stat='identity',aes(fill=continent)) +
    scale_fill_brewer(palette = 'Blues') #내림차순
