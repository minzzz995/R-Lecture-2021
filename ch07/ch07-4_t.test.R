...
n <- length(weight)
h0 <- 2800 #모집단의 평균(기존에 여아의 몸무게 평균)
t.t <- (barx - h0) / (s / sqrt(n)) #검정통계량

alpha <- 0.05 #95% 신뢰수준을 가짐 
c.u <- qt(1-alpha, df=n-1) #누적확률이 0.95가 되는 점
p.value <- 1 - pt(t.t, df=n-1) #검정통계량 2.23보다 클 확률 

#R에서 제공하는 t-test
t.test(weight, mu=2800, alternative="greater") #우리가 구하려고 하는 것 : weight /모집단의 평균 : mu
# p-value만 보고 0.05보다 크다 작다만 확인하기 !!
...
polygon(c(c.u, x[x>c.u], 3), c(0, y[x>c.u], 0), col=2) #c.u지점을 찍어 겹치는 부분을 그려라
...
# 정규성 테스트
shapiro.test(boy$weight) # 0.01이므로 0.05보다 작아 정규성이 없다고 할 수 있음
qqnorm(boy$weight)
qqline(boy$weight)
shapiro.test(girl$weight) # 0.01이므로 0.20보다 커 정규성이 있있다고 할 수 있음
qqnorm(girl$weight)
qqline(girl$weight)
...
var.test(weight ~ gender, data=data) # p-value = 0.06764로 같다고 할 수 있다
...
alpha <- 0.05
qt(alpha, df=16)
pt(t.t, df=16)    # 검정통계량으로부터 구한 유의확률 = 0.0003501266

t.test(data$Prior, data$Post, paired=T, alternative="less") #작다라고 하는 것이 성립된다